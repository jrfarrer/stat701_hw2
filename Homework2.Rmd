...
packages <- c('tidyverse','ggthemes','knitr','extrafont','broom','aod','leaps','bestglm',
              'GGally','xtable','pander','descr','plotROC', 'ROCR','pROC', 'corrplot')
...
We then remove `r framingham_cleaning %>% filter(!complete.cases(.)) %>% count() %>% unlist()` that have missing values.
...
(fit2_summary <- summary(fit2)) %>% xtable()
```
...
The $p$-value for adding the variable sex is `r chi_sq_fit2`. The $p$-value is not the same as the Wald Test. The Wald Test and the Likelihood Test both hold for this new model.
...
This method is not guaranteed to include only variables with $p$-values less than 0.05. In this example, the best model includes frw which the summary shows is not significant at the 0.05 level. Thus, the best model returned by exhaustive search is not the same as the model returned by backwards elimination.

```{r results = 'asis'}
final_model <- exhaustive_model$BestModel
summary(final_model) %>% xtable()
```
...
I would choose fit2, but fit1 and fit2 are nearly the same. I would want to you fit2 slightly have the 0.5 probability threshold.
...
The range of the x-axis is from 0.5 to 0.7 which reflects the range of the fitted values from final model for 1 b). We know that model has a range from `r min(final_model$fitted)` to `r max(final_model$fitted)` which when exponentiated and converted to a probability is `r round(exp(min(final_model$fitted)) / (1 + exp(min(final_model$fitted))), 2) - 0.01` to `r round(exp(max(final_model$fitted)) / (1 + exp(max(final_model$fitted))), 2)`.
...
The Bayes rule classifier with $\frac{a_{1,0}}{a_{0,1}} = 1$ performs much better and indeed the misclassification rate as the threshold increases.
...
There are `r sum(is.na(bills_train2$status))` values of status that are `NA`. Based on the assignments comment "All other outcomes are failures" we will assume these are failures to pass rather than missing values. We will remove the observation without day of week so that different models are comparable.
...
```{r results = 'asis'}
pander(summary(bills_train3), missing = "")
``` 
...
2. Whether the bill's sponser in leadership does not have as a large of effect as expected
3. The number of amendments variable looks to be relatively powerful (four column from the right above)
...
We also look at the relationship between number of amendments and whether the bills passed. We note if bills with at least one amendment had `r round(100 * bills_train3 %>% filter(num_amendments > 0) %>% summarise(pass_rate = sum(status2 == 1) / n()),2) %>% unlist()`% pass rate vs `r round(100 * bills_train3 %>% filter(num_amendments == 0) %>% summarise(pass_rate = sum(status2 == 1) / n()),2) %>% unlist()`% for bills without an amendment.

```{r, results='asis'}
pander(descr::CrossTable(bills_train3$num_amendments, bills_train3$status2, prop.chisq=FALSE, prop.t = FALSE, chisq = TRUE, prop.c = F, prop.r = F,dnn = c("Number of Amendments","Status")), split.table = Inf, big.mark = ',')
```
...
```{r results = 'asis'}
bills_fit1 <- glm(status2 ~ session + dow + is_sponsor_in_leadership2 + sponsor_party +
                      num_cosponsors + num_d_cosponsors + num_r_cosponsors + title_word_count + num_amendments +
                      num_originating_committee_cosponsors + num_originating_committee_cosponsors_r +
                      num_originating_committee_cosponsors_d, data = bills_train3, family = binomial)
bills_fit2 <- glm(status2 ~ session + dow + is_sponsor_in_leadership2 + sponsor_party +
                      num_cosponsors + num_d_cosponsors + title_word_count + num_amendments +
                      num_originating_committee_cosponsors + num_originating_committee_cosponsors_r, 
                      data = bills_train3, family = binomial)
bills_fit3 <- glm(status2 ~ session + dow + is_sponsor_in_leadership2 + sponsor_party +
                      num_cosponsors + title_word_count + num_amendments +
                      num_originating_committee_cosponsors + num_originating_committee_cosponsors_r, 
                      data = bills_train3, family = binomial)
bills_fit4 <- glm(status2 ~ session + dow + is_sponsor_in_leadership2 + sponsor_party +
                      num_cosponsors + title_word_count + num_amendments +
                      num_originating_committee_cosponsors, data = bills_train3, family = binomial)
...
bills_fit6 <- glm(status2 ~ session + is_sponsor_in_leadership2 + sponsor_party +
                      title_word_count + num_amendments +
                      num_originating_committee_cosponsors, data = bills_train3, family = binomial)
bills_fit7 <- glm(status2 ~ session + sponsor_party + title_word_count + num_amendments +
                      num_originating_committee_cosponsors, data = bills_train3, family = binomial)
bills_fit8 <- glm(status2 ~ session + sponsor_party + title_word_count + num_amendments, 
                  data = bills_train3, family = binomial)
xtable::xtable(bills_fit8)
```
...
best_glm_bills <- glmulti::glmulti(status2 ~ session + dow + is_sponsor_in_leadership2 + sponsor_party +
                      ...
                      data = bills_train3, family = binomial, crit = aic, level = 1, plotty = FALSE, report = FALSE)
```

`glmulit` returns a best model wiht the equations `r gsub('  ', '', paste0(summary(best_glm_bills)$bestmodel, collapse = ""))`.
...
data_frame(
    ...
    , AUC = c(auc(roc(bills_train3$status2, bills_fit1$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit2$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit3$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit4$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit5$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit6$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit8$fitted))[1]
            , auc(roc(bills_train3$status2, bills_fit7$fitted))[1])
...
data_frame(
    ...
    , `CV estimate of Prediction Rrror` = c(boot::cv.glm(bills_train3, bills_fit8, K = 10)$delta[[1]],
                                            boot::cv.glm(bills_train3, bills_fit7, K = 10)$delta[[1]])
) %>% 
pander()
```
...
We propose a loss function such that  $\frac{a_{1,0}}{a_{0,1}} = 5$ in order to minimize false negatives. A bill passing in a very rare event (unfortunately) so we want to maximize the negative predictive value (incidence of false negatives). We see no difference in weighted MCE.
    
```{r results = 'asis'}
data_frame(
    status = bills_train3$status2
...
```{r, results='asis'}
confusion_bills <- table(bills_train3$status2, ifelse(predict(bills_fit8, type = 'response') > .5, 1, 0))

pander(descr::CrossTable(bills_train3$status2, ifelse(predict(bills_fit8, type = 'response') > .5, 1, 0), prop.chisq=FALSE, prop.r = F, prop.c = F, prop.t = TRUE, chisq = TRUE, dnn = c("Status","Prediction")), split.table = Inf, big.mark = ',')
```
...
bills_test_pred <- ifelse(predict(bills_fit8, bills_test3, type = "response") > 0.5, 1, 0)

confusion_test_bills <- table(bills_test3$status2, bills_test_pred)

pander(descr::CrossTable(bills_test3$status2, bills_test_pred, prop.chisq=FALSE, prop.r = F, prop.c = F, prop.t = TRUE, chisq = TRUE, dnn = c("Status","Prediction")), split.table = Inf, big.mark = ',')
```
